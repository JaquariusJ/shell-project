...
nfsPath=/root/cgn/nfs/redis/data
#构建yaml
echo "---------------------构建yaml文件---------------------"
...
echo "kubectl create cm ${deployName}-conf --from-file=./project/conf/redis.conf -n ${namespace}"
kubectl create cm ${deployName}-conf --from-file=./project/conf/redis.conf -n ${namespace}
