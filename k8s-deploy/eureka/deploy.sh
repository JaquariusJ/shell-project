...
projectName=cgn
deployName=eureka
mirror="${dockerRegistry}/${projectName}-${deployName}-svc:v_"$(date +"%Y%m%d%H%M%S")
port=9000
namespace=4pd
replicas=1
cpuRequest=500m
...
echo "kubectl create cm ${deployName}-conf --from-file=./project/conf -n ${namespace}"
kubectl create cm ${deployName}-conf --from-file=./project/conf -n ${namespace}
